...
#define NUM_FRAMES 256                   // change number of required frames here
...
void translateAddresses(int* logicalAddresses, int addressCount, queue *fifoQueue) {
    ...
    for (int k = 0; k < addressCount; k++) {
        ...
        if (isTLBHit(pageNumber)) {
            ...
            // If not, handle page fault.
            if (pageTable[pageNumber].valid && pageTable[pageNumber].frameNumber != EMPTY) {

                // nothing happens, page hit
                continue;
                frameNumber = pageTable[pageNumber].frameNumber;
                goto print_out;
                
            } 
            else if (pageTable[pageNumber].valid && pageTable[pageNumber].frameNumber == EMPTY) {
                pageFaults++;
                
                frameNumber = k % NUM_FRAMES;

                // getting values from backing store
